...
void TreeLSTM::init(Rand& rnd, const double scale){
...
void TreeLSTM::forward(const MatD& xt, TreeLSTM::State* parent, TreeLSTM::State* left, TreeLSTM::State* right){
  parent->i = this->bi + this->Wxi*xt + this->WhiL*left->h + this->WhiR*right->h;
  parent->fl = this->bfl + this->Wxfl*xt + this->WhflL*left->h + this->WhflR*right->h;
  parent->fr = this->bfr + this->Wxfr*xt + this->WhfrL*left->h + this->WhfrR*right->h;
  parent->o = this->bo + this->Wxo*xt + this->WhoL*left->h + this->WhoR*right->h;
  parent->u = this->bu + this->Wxu*xt + this->WhuL*left->h + this->WhuR*right->h;
...
void TreeLSTM::backward(TreeLSTM::State* parent, TreeLSTM::State* left, TreeLSTM::State* right, TreeLSTM::Grad& grad, const MatD& xt){
  MatD delo, deli, delu, delfl, delfr;

  parent->delc.array() += ActFunc::tanhPrime(parent->cTanh).array()*parent->delh.array()*parent->o.array();
  left->delc = parent->delc.array()*parent->fl.array();
  right->delc = parent->delc.array()*parent->fr.array();
...
void TreeLSTM::sgd(const TreeLSTM::Grad& grad, const double learningRate){
